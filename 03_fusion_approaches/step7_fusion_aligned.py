#!/usr/bin/env python3
"""
Step 7 ALIGNED: Multi-Modal Fusion with Feature Count Alignment
Author: Senior ML Engineer & Biology Researcher
Date: 2025-01-22

Goal: Maximize fusion AUC using feature counts aligned with individual model performance
- NO DATA LEAKAGE (all feature selection inside CV)
- Feature counts based on step6 individual optimization results
- Advanced fusion: weighted, stacking, simple average
- Ablation study to quantify each modality's contribution
- Fixed bootstrap confidence intervals

Changes from original:
- Expression: 375 features (was 5000)
- Methylation: 2000 features (was 1000)
- Mutation: 1000 features (unchanged)
- Protein: 100 features (was 90)

Target: AUC 0.76-0.77 (improvement from feature alignment)
"""
...
def run_phase1_strategic(modalities_data, y_train, sample_ids, class_weights):
    ...
    # Define configs aligned with individual model best results
    # Based on step6 optimization: expression=375, methylation=2000, mutation=1000, protein=100
    phase1_configs = {
        'expression': [375, 500, 250, 450, 300, 600],  # Best individual was 375
        'methylation': [2000, 1500, 2500, 1000, 3000, 1250],  # Best individual was 2000
        'mutation': [1000, 750, 1250, 500, 300, 800],  # Best individual was 1000
        'protein': [100, 75, 125, 90, 110, 85]  # Best individual was 100
    }
    ...
    for modality in ['expression', 'methylation', 'protein', 'mutation']:
        ...
        for n_features in phase1_configs[modality]:
            ...
            print(f"  Testing {n_features} features...", end='', flush=True)
            fold_aucs = []
...
def main():
    ...
    output_dir = f'{data_dir}/v2_fusion_results'
    ...
    # Save results
    with open(f'{output_dir}/v2_fusion_results.json', 'w') as f:
        json.dump(results, f, indent=2)
    
    # Comprehensive final summary
    print("\n" + "="*80)
    print("FINAL FUSION MODEL SUMMARY")
    print("="*80)
    
    print("\nBEST FEATURE CONFIGURATIONS:")
    for modality, n_features in best_configs.items():
        auc = refined_results[modality][n_features]
        print(f"  {modality.upper()}: {n_features} features (Individual AUC: {auc:.3f})")
    ...
    print(f"\nBEST FUSION: {best_method} (AUC: {best_auc:.3f})")
    
    if 'weighted' in fusion_results and fusion_results['weighted']:
        print("\nMODALITY WEIGHTS (approximate from performance):")
        # Note: actual weights vary by fold but this gives an idea
        total_auc = sum(refined_results[m][best_configs[m]] for m in best_configs)
        for modality, n_features in best_configs.items():
            weight = refined_results[modality][n_features] / total_auc
            print(f"  {modality}: {weight:.1%}")
    
    print("\nABLATION STUDY - MODALITY CONTRIBUTIONS:")
    for key, value in ablation_results.items():
        if key == 'full':
            print(f"  Full model: {value:.3f}")
        else:
            modality = key.replace('without_', '')
            contribution = ablation_results['full'] - value
            print(f"  {modality} contribution: +{contribution:.3f} AUC")
    ...
    if mean_auc:
        ...
        print("  Bootstrap failed - insufficient data")
    
    print("\nPERFORMANCE SUMMARY:")
    print(f"  Best fusion AUC: {best_auc:.3f}")
    print(f"  Improvement over best individual: +{best_auc - max(refined_results[m][best_configs[m]] for m in best_configs):.3f}")
    
    print(f"\nTotal runtime: {total_time/60:.1f} minutes")
    print(f"Results saved to: {output_dir}/v2_fusion_results.json")
