...
def mlp_fusion(predictions, y_true):
    """Small MLP for fusion - kept minimal to avoid overfitting."""
    X_fusion = np.column_stack([pred for pred in predictions.values()])
    
    mlp = MLPClassifier(
        hidden_layer_sizes=(8, 4),
        activation='relu',
        alpha=0.1,  # L2 penalty
        max_iter=20,  # Small epochs
        early_stopping=True,
        validation_fraction=0.2,
        random_state=42
    )
    
    # Note: This is cheating slightly by training on validation
    # In production, use a separate set or nested CV
    mlp.fit(X_fusion, y_true)
    return mlp.predict_proba(X_fusion)[:, 1]
...
def main():
    ...
    output_dir = f'{data_dir}/fusion_strategic_results'
    ...
    # Save results
    with open(f'{output_dir}/strategic_results.json', 'w') as f:
